...
somebody else creates new revision, we are not albe to commit ours. Rebase is solving this problem without loosing
changes we made.
...
our old parent. This represents changes made by us. These sets has to be somehow merged and shown to a user in order
to see, haw the final result of merge will look like and also in order to influence the result of this merge.
...
constraint any time. Rename is before create because you can create then the object of the same name as an old one,
...
implemented in {\tt UserInterfaceIO}. Here the user can delete or modify the modifications. When the editing part is
finished, the corrected list is processed and all modifications are performed. Modifications made by us can be editted
simply as they do not exist in the new changeset and are in format readable by the {\tt Parser}. Modifications made by
...
occures old changeset is aborted and we can continue in work in the new changeset or commit it.
