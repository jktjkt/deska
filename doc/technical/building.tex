...
\begin{itemize}
    \item Red Hat Enterprise Linux 5 (release 5.6 or newer, including derivatives like Scientific Linux)
...
\begin{itemize}
    ...
    \item Boost Date Time (1.41 or newer)
    ...
    \item GNU Readline (5.1 or newer; in case of RHEL5, we also require the {\tt termcap} library version 2.0.8 or
        newer)
    \item The {\tt libebt} library (optional, shipped in the sources)
    \item The {\tt json\_spirit} library (optional, shipped in the sources)
...
\begin{itemize}
    \item PostgreSQL (9.0 or newer)
    \item Python (2.6, or Python 2.4 with the {\tt simplejson} module version 2.1.0)
    \item PsycoPg2 (2.0.14 or newer)
    \item The {\tt pg-python} procedural language (1.0.0 or later; not to be confused with the PlPython package)
    \item GitPython (the 0.1 branch; tested with 0.1.7, later branches have changed the API in an incompatible way)
\end{itemize}
...
\begin{itemize}
    \item PgTAP unit test framework for Postgres (0.25 or later)
\end{itemize}

Fully automated test runs might require the following packages to be present and configured:

\begin{itemize}
    \item CMake (2.8 or later provides proper Git integration for nightly builds)
...
\begin{itemize}
    ...
    \item The {\tt pygments} Python package
    \item The {\tt subfiles} \LaTeX~package (optional, shipped with Deska)
\end{itemize}

\subsubsection{Getting the Sources}

Apart from the stable releases available from the project homepage (FIXME ref), the sources are also available from an
...
interaction with a remote PostgreSQL database:

\begin{minted}{console}
$ cd deska/
$ mkdir _build
$ cd _build/
$ cmake -DCMAKE_INSTALL_PREFIX=/opt -DRUN_SQL_TESTS=1 ..
