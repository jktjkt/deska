...
--select large_modul_diff_set_attributes(30,60);
--drop table diff_data;

create or replace function large_modul_diff_set_attributes(from_version bigint, to_version bigint)
returns setof diff_set_attribute_type
as
$$
begin
perform init_diff(from_version,to_version);
return query select * from large_modul_diff_deleted();
end
$$
language plpgsql;
