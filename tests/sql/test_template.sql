...
BEGIN
	...
	PREPARE retresolved_data AS SELECT purchase,warranty,template FROM hardware_template_resolved_object_data('purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty,template FROM hardware_template_resolved_object_data('ram_cpu_purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, template FROM hardware_template_resolved_object_data('template_all');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, template FROM hardware_template_resolved_object_data('purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, note_hardware, template FROM hardware_template_resolved_object_data('template_all');
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template FROM hardware_resolved_object_data('templated_hardware_1');
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template FROM hardware_resolved_object_data('templated_hardware_2');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of templated hardware are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, note_hardware, template FROM hardware_template_resolved_object_data('purchase_warranty_template');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template FROM hardware_template_resolved_object_data('ram_cpu_purchase_warranty_template');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template FROM hardware_template_resolved_object_data('template_all');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT vendor_get_name(vendor), purchase,warranty, ram, cpu_num, note_hardware, hardware_template_get_name(template) FROM production.hardware WHERE name = 'templated_hardware_2';
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template FROM old_version_resolved_data;
...
BEGIN
	...
	PREPARE retresolved_data AS SELECT purchase, warranty, template FROM hardware_resolved_object_data('templated_hardware');
