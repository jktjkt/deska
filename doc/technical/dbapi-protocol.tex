...
this matter.  For a quick reference, please check the overview tables in \secref{sec:json-data-types-reference}.
...
see \secref{sec:json-extending} for details.
...
described in \secref{sec:api-group-dbscheme}.  These commands inform the user about what kinds of top-level objects are
defined, what relations exist among them and which attributes belong to each object.  Second part of the API
(\secref{sec:api-group-data-retrieval}) is concerned with returning actual object data.  Going further, the third
part (\secref{sec:api-group-data-modification}) focuses on manipulating the data in the database, while the fourth
part (\secref{sec:api-group-vcs}) allows the user to manage the individual changesets and eventually commit the
results, or provide a frozen view of the DB state for consistency.  The fifth part (\secref{sec:api-group-history})
discusses browsing the revision history and the last part (\secref{sec:api-group-conf-generators}) controls
...
make them permanent, she shall use the \deskaFuncRef{commitChangeset} to commit them into a persistent revision.  The
changesets can be also explicitly locked and unlocked through the \deskaFuncRef{lockCurrentChangeset} and
\deskaFuncRef{unlockCurrentChangeset} methods.
...
this purpose, the \deskaFuncRef{freezeView} and \deskaFuncRef{unFreezeView}, see \secref{sec:api-revision-semantics} for
details.
...
still valid via the diffing functions (see \secref{sec:api-group-history}) and apply them in a newly created
changeset.
...
server during commit, the privileged user shall substitute calls to the \deskaFuncRef{commitChangeset} through the
\deskaFuncRef{restoringCommit} function.
...
\deskaFuncRef{setAttribute}.  Certain attributes accept a set of identifiers; for an effective way of modifying these
items, see \deskaFuncRef{setAttributeInsert} and \deskaFuncRef{setAttributeRemove}.
...
or any similar revision (or changeset) metadata.  For filters against an {\tt identifier\_set} data type, only the
existing {\tt columnEq} and {\tt columnNe} and new {\tt columnContains} and {\tt columnNotContains} comparison operators
are allowed.  The greater/lesser operators are not allowed in expressions which reference identifier
sets.\footnote{Mixing these operators in a compound filter, as described later in this section, is completely valid and
supported.}
...
automatic name assignment via \deskaFuncRef{createObject}.
...
The diffing functions (see \secref{sec:api-group-history}) sorts the results according to special rules, see their
...
\deskaFunc{kindInstances}{, "kindName": "something", "revision": "r123", "filter": \ldots}
    { ["a", "b", "c"] }
    ...
    depends on the usual rules --- see \secref{sec:api-revision-semantics} for details.  Another optional argument, {\tt
...
\deskaFunc{resolvedObjectData}{, "kindName": "kind", "objectName": "object", "revision": "r123"}
    ...
    this function operates exactly as the \deskaFuncRef{objectData}.
...
\deskaFunc{multipleResolvedObjectData}{, "kindName": "kind", "filter": Filter, "revision": "r123"}
    ...
    \deskaFuncRef{resolvedDataDifference} function, and the behavior for kinds which are not templatized is the same as
    the \deskaFuncRef{multipleObjectData} function.
...
\deskaFunc{multipleResolvedObjectDataWithOrigin}{, "kindName": "kind", "filter": Filter, "revision": "r123"}
    ...
    {This is the batched version of the \deskaFuncRef{resolvedObjectDataWithOrigin}; the only difference between these
    ...
    Similar to \deskaFuncRef{resolvedObjectDataWithOrigin}, the name of an object providing a value can be {\tt null},
...
\deskaFunc{setAttributeInsert}{, "kindName": "something", "objectName": "name", "attributeName": "name", "attributeData": "id"}
    {}
...
\deskaFunc{setAttributeRemove}{, "kindName": "something", "objectName": "name", "attributeName": "name", "attributeData": "id"}
    {}
...
\deskaFunc{applyBatchedChanges}{, "modifications": [ \\
    ...
    \{"command": "setAttribute", "kindName": "k5", "objectName": "o5", \\
    "attributeName": "a5", "attributeData": "new"\}, \\
    \{"command": "setAttributeInsert", "kindName": "k6", "objectName": "o6", \\
    "attributeName": "a6", "attributeData": "one-id"\}, \\
    \{"command": "setAttributeRemove", "kindName": "k7", "objectName": "o7", \\
    "attributeName": "a7", "attributeData": "one-id"\} \\
    ]}
    {}
    ...
    \secref{sec:api-group-history} return.)
    }
...
Querying History}, \secref{sec:api-group-history}.
...
\deskaFunc{pendingChangesets}{, "filter": \ldots}
    {[
...
\deskaFunc{lockCurrentChangeset}{}{}
    ...
    created the lock.  Other sessions can not perform any modifications of that particular changeset, including attching
    a locked changeset.  The lock remains in effect until either the session terminates, or
    \deskaFuncRef{unlockCurrentChangeset} on the same changeset is called, or the changeset ceases to exist (via either
    \deskaFuncRef{abortCurrentChangeset} or a successful call to \deskaFuncRef{commitChangeset}).

    The lock operation can fail if the changeset is already locked by current session, or if another session has
    attached the ``current'' changeset, effectively detaching this session.
    }

\deskaFunc{unlockCurrentChangeset}{}{}
    {Unlock current changeset}
    {This function marks the current changeset as unlocked.  It is an error to attempt to unlock a changeset which has
    not been locked.}
...
\deskaFunc{freezeView}{}{}
    ...
    database is. See \secref{sec:api-revision-semantics} for details.
...
\deskaFunc{unFreezeView}{}{}
    ...
    {This function removes the frozen view of the database state.  See \deskaFuncRef{freezeView} and
    \secref{sec:api-revision-semantics} for details.}

\deskaFunc{restoringCommit}{, "commitMessage": "message", "author": "user", "timestamp": "2011-04-07 17:22:33"}
    {"r123"}
    {Extended version of \deskaFuncRef{commitChangeset} that allows specifying additional revision metadata}
    ...
    This function is similar to the \deskaFuncRef{commitChangeset}, but in addition accepts two additional arguments, the {\tt
...
\deskaFunc{listRevisions}{, "filter": \ldots}
    {[
