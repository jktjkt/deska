...
Using this relations, a logical object is essentially composed of many smaller objects, where each of the ``contained''
...
serial number) and {\tt placement} (for the physical space it occupies in a rack).  An Ethernet switch would typically
...
already a switch called {\tt foobar} and one attempts to create a {\tt placement} of the same kind, it will immediately
get tied to the switch with the same name.  Constraint checks are in place that forbid creating a {\tt switch} and a
