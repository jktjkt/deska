...
Temporary changesets could be closed by calling commitChangeset function. This functions closes temporary changeset that is attached to the connection and creates new version. Closing of the changeset is done by deleting of row with record about it from the changeset table. New version is created by inserting a new row into version table. The row has information about id of the new version, author, id of changeset from which was this new version created.\\
Temporary changeset could be closed also by calling abortChangeset. This function just closes the attached temporary changeset.\\
