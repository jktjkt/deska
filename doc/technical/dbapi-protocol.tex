...
database, after all, is built on PostgreSQL~\cite{postgresql}, which already provides a library which can be used to access database
...
XML-RPC family.  For the Deska DB API, we have chosen to use JSON~\cite{json}, a fairly recent arrival to the already broad family
...
rather on some ``magic'' value.  At this time, only one filter type is implemented, the {\tt \{"specialCondition":
"last"\}}, which matches an object which has
...
\deskaFunc{lockCurrentChangeset}{}{}
    ...
    \deskaFuncRef{abortCurrentChangeset} or a successful call to \deskaFuncRef{commitChangeset}).  The locks are
    reentrant, and in order to make sure the changeset is unlocked, the number of calls to
    \deskaFuncRef{unlockCurrentChangeset} and to this function have to match.
...
\deskaFunc{restoringCommit}{, "commitMessage": "message", "author": "user", "timestamp": "2011-04-07 17:22:33"}
    ...
    In contrast to the \deskaFuncRef{commitChangeset}, calling this function does {\em not} trigger an automatic
    regeneration of configuration output, as we assume that producing the history again might lead to undesired
    interactions with the live system.}
