...
These functions and tables are genearated by SQL generator (see \secref{sec:sql-generator}) during installation of the Deska database. How the generated functions and tables work together to fulfil expectations from DBAPI (see~\secref{sec:dbapi-protocol}) is described in \secref{sec:deska-db-data-manipulation}.
...
After Deska server starts, it tries to connect to Deska database using Psycopg2 python modul.
Parameters of connection are taken from Deska server parameters on start.
When connection is successfully established, it is kept until Deska server is terminated.

\subsection{Command running}
For communication with the deska-cli, the JSON-based DBAPI (see~\secref{sec:dbapi-protocol} is used. On server side, this JSON is tranformed into python structure of dicts and lists. Some checks are performed, like if command or number of arguments are right.
Most of commands have their implementation inside the database, so the proper db function is called and its result is retuned. There is no special logic in this case, parameters are just checked, some defaults are added if needed and db exceptions are catched.

But there are some functions implemented in deska-server, like function for view freezing, which just starts or ends transaction, with isolation
level read committed, to perform the freezing of view.
Also showConfigDiff is not implemented in the Deska database. Special is commitChangeset, which has database implementation,
but also needs to do generation of configuration outside the database (similar to showConfigDiff).
These two functions that run Configuration genarators (\secref{sec:config-generators}), works with read commited isolation level transaction.
At first commitChangeset in Deska database is run, then Configuration generator produces configuration.
...
The combination of these 2 types is applyBatchedChangeset, which starts transaction at the begining, then perform its arguments, as it was a single
...
The Deska server relies heavily on the authentication/authorization features offered by the SSH server, and on the
...
There are tables, specified by user before build of Deska database, tables for history tracking and many function for data manipulation.
