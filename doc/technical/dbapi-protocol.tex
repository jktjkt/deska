...
\begin{itemize}
    ...
    \item Create a specialization of the {\tt template<> struct JsonConversionTraits<>}, its {\tt extract()} and {\tt
        toJson()} methods, which are responsible for proper serialization and de-serialization between native C++ data
        types and JSON
...
Under certain circumstances, it might be useful to allow querying of objects using indirect criteria, like ``return all
...
{\tt \{"condition": }ExpressionKind{\tt, "kind": "interface", "attribute": "vlan", "value": }Value{\tt \}}

The database server will parse this filter and attempt to issue a corresponding {\tt JOIN} statement.  Using these
``foreign'' attributes is allowed only in case that both kinds are connected via a compatible relation.  If there is no
such relation, the Deska server returns exception {\tt FilterError}.
...
\deskaFunc{kindInstances}{, "kindName": "something", "revision": "r123"}
    ...
    selected object kind, but can also follow defined object relations to create more complex queries.}
...
\deskaFunc{objectData}{, "kindName": "kind", "objectName": "object", "revision": "r123"}
    ...
    \deskaFuncRef{multipleResolvedObjectData}.
    }
...
\deskaFunc{multipleObjectData}{, "kindName": "kind", "filter": Filter, "revision": "r123"}
    ...
    \deskaFuncRef{objectData} function.}
...
\deskaFunc{multipleResolvedObjectData}{, "kindName": "kind", "filter": Filter, "revision": "r123"}
    ...
    {This is the batched version of the \deskaFuncRef{resolvedObjectData}.}
...
\deskaFunc{dataDifferenceInTemporaryChangeset}{, "changeset": "tmp123"}
    ...
    {Return differences between the database state in the specified changeset}
...
\deskaFunc{resolvedDataDifferenceInTemporaryChangeset}{, "changeset": "tmp123"}
    ...
    {Return differences between the resolved database state in the specified changeset}
