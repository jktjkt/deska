...
in order to perform restore. This is checket before any actions are made.
...
context in which the parser currently is. It is based on many sub parsers stored in maps and calfled depending on the current context.
...
like parsr of timestamps, revisions IDs, temporary changeset IDs and so on. You can see reference manual for more details.
...
one attribute. It menas, that parse accepts any attribute of a specific kind, but not a list of them.
For parsing set of thees pairs, use some {\tt boost::spirit} operator like kleene star.
...
Function words parser is a stand-alone praser outside of the main parsing loop. As we wrote, this one parses function words
...
You can see, that error handler are there tha same as in {\tt AttributesSettingParser} and has also the same function here.
...
and then use these tokens as possible completion patterns. Moreover when the error occures while parsing an attribute
