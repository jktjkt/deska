...
BEGIN
	...
	PERFORM hardware_template_set_template_hardware('purchase_warranty_template','purchase_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty,template_hardware FROM hardware_template_resolved_object_data('purchase_warranty_template');
	...
	PERFORM hardware_template_set_template_hardware('ram_cpu_purchase_warranty_template','purchase_warranty_template');
	PERFORM commitChangeset('1');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty,template_hardware FROM hardware_template_resolved_object_data('ram_cpu_purchase_warranty_template');
	...
	PERFORM hardware_template_set_note_hardware('template_all','blabla note');
	PERFORM hardware_template_set_template_hardware('template_all','ram_cpu_purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, template_hardware FROM hardware_template_resolved_object_data('template_all');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, template_hardware FROM hardware_template_resolved_object_data('purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM hardware_template_resolved_object_data('template_all');
	...
	PERFORM hardware_set_template_hardware('templated_hardware_1','template_all');
	...
	PERFORM hardware_set_template_hardware('templated_hardware_2','ram_cpu_purchase_warranty_template');
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM hardware_resolved_object_data('templated_hardware_1');
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM hardware_resolved_object_data('templated_hardware_2');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of templated hardware are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;

	PERFORM startChangeset();
	PERFORM hardware_template_set_note_hardware('purchase_template','note2');
	...
	PREPARE retresolved_data AS SELECT purchase,note_hardware FROM hardware_template WHERE name = 'purchase_template';
	...
	PREPARE retresolved_data AS SELECT purchase,warranty, note_hardware, template_hardware FROM hardware_template_resolved_object_data('purchase_warranty_template');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM hardware_template_resolved_object_data('ram_cpu_purchase_warranty_template');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM hardware_template_resolved_object_data('template_all');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware_template after change in template are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;

	PERFORM startChangeset();
	PERFORM hardware_template_set_note_hardware('purchase_template','note3');
	PERFORM commitChangeset('1');
	...
	PREPARE retresolved_data AS SELECT vendor_get_name(vendor), purchase,warranty, ram, cpu_num, note_hardware, hardware_template_get_name(template_hardware) FROM production.hardware WHERE name = 'templated_hardware_2';
	...
	PREPARE expresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note, template FROM pgtap.test_hardware WHERE name = 'templated_hardware_2' AND version = 6;
	CREATE TEMPORARY TABLE old_version_resolved_data AS SELECT * FROM hardware_resolved_object_data('templated_hardware_2',old_version);
	PREPARE retresolved_data AS SELECT vendor, purchase,warranty, ram, cpu_num, note_hardware, template_hardware FROM old_version_resolved_data;
...
BEGIN
	...
	PERFORM hardware_set_template_hardware('templated_hardware','first_template');
	...
	PREPARE retresolved_data AS SELECT purchase, warranty, template_hardware FROM hardware_resolved_object_data('templated_hardware');
	RETURN NEXT results_eq( 'expresolved_data', 'retresolved_data', 'resolved template data of hardware are ok' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;

	PERFORM startChangeset();
	PERFORM hardware_set_template_hardware('templated_hardware','second_template');
	PERFORM commitChangeset('3');
