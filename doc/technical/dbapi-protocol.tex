...
\begin{longtable}{ p{60mm} p{90mm} }
    ...
    {\tt Deska::Db::ObjectModification} &
        ...
        or \newline
        {\tt \{"command": "removeAttribute", "kindName": }Indetifier{\tt, "objectName": }Identifier{\tt,
        "attributeName": }Identifier{\tt \}}, \newline
        or \newline
...
resolving inherited values from object templates, and also \deskaFuncRef{findOverriddenAttrs} and
\deskaFuncRef{findNonOverriddenAttrs} for assessing the impact of changes to an individual object's attribute.
...
make them permanent, she shall use the \deskaFuncRef{commitChangeset} to commit them.
...
renamed via the \deskaFuncRef{renameObject}.  Attribute values are set via the \deskaFuncRef{setAttribute} call, which
can be undone with \deskaFuncRef{removeAttribute}.
...
the current query applies.  When the version is not specified, behavior of these commands varies depending on whether
there is any active changeset associated with current session.  When the session is in the default state, ie. at the
beginning of the connection when no VCS management has been attempted yet, or after a
\deskaFuncRef{detachFromCurrentChangeset} succeeded, the implicit revision is fixed at ``latest committed at the time
the session started''.  After each successful commit of a permanent revision, this number gets updated to the revision
number of that revision.
