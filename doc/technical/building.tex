...
\begin{itemize}
    ...
    \item {\tt deska}, for the shared libraries
    \item {\tt deska-devel}, for development files like the C++ headers
    %\item {\tt deska-docs}, for a copy of the documentation
\end{itemize}
...
These packages indicate their desired dependencies correctly, and if YUM has the required repositories available, the
dependency resolution works automatically.   However, if the target system is for some reason configured in a different
way (very likely due to a few external repositories), the installation might fail with an error message about
unsatisfiable dependencies.  In such cases, installing the dependency might help:

\begin{minted}{console}
# yum localinstall path/to/external/deps/*.rpm
\end{minted}

\todo{Honza: list of the external packages}

A typical user's workstation or a laptop does not have to contain the whole Deska system.  To install just the Deska
CLI, proceed in the following way, installing packages {\tt deska} and {\tt deska-client}:

\begin{minted}{console}
# yum localinstall path/to/deska{,-client}-1.0-1.rpm
\end{minted}
