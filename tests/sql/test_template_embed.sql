...
BEGIN
	...
	INSERT INTO pgtap.test_interface_template (name, host, note, template, version) VALUES ('inf_template_note_host', 'host2', 'interface', 'inf_note_template', 2);
	INSERT INTO pgtap.test_interface (name, host, note, template, version) VALUES ('host1->eth1', 'host1', 'interface', 'inf_template_note_host',3);
	INSERT INTO pgtap.test_interface (name, host, note, template, version) VALUES ('host2->eth1', 'host2', 'interface', 'inf_template_note_host',4);
	...
	PERFORM interface_template_set_host('inf_template_note_host','host2');
	PERFORM interface_template_set_template('inf_template_note_host','inf_note_template');
	PERFORM commitchangeset('2');

	PREPARE expresolved_data AS SELECT host, note, template FROM pgtap.test_interface_template WHERE name = 'inf_template_note_host' AND version = 2;
	PREPARE retresolved_data AS SELECT host, note, template FROM interface_template_resolved_data('inf_template_note_host');
	RETURN NEXT results_eq( 'retresolved_data', 'expresolved_data', 'resolved template data are ok' );	
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
	...
	PERFORM interface_set_template('host1->eth1','inf_template_note_host');
	...
	PERFORM startchangeset();
	PERFORM interface_set_host('host1->eth1',null);
	PERFORM commitchangeset('4');

	PREPARE expresolved_data AS SELECT host, note, template FROM pgtap.test_interface WHERE name = 'host2->eth1' AND version = 4;
	PREPARE retresolved_data AS SELECT host, note, template FROM interface_resolved_data('host2->eth1');
	RETURN NEXT results_eq( 'retresolved_data', 'expresolved_data', 'resolved object data are ok - 2 levels' );
	DEALLOCATE expresolved_data;
	DEALLOCATE retresolved_data;
