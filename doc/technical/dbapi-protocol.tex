...
server, the privileged user shall call the \deskaFuncRef{enableRestoreMode} function.
...
\deskaFunc{commitChangeset}{, "commitMessage": "message"}
    ...
    When in the {\em restore mode} (see \deskaFuncRef{enableRestoreMode}), this function accepts two additional
    arguments, an {\tt author} and a {\tt timestamp} one.  These will then get used as the source of information for
    specifying the revision metadata.}
...
\deskaFunc{enableRestoreMode}{}{}
    {Enable specifying revision arguments by user}
    ...
    After this function has been called, the \deskaFuncRef{commitChangeset} accepts two additional arguments, the {\tt
    author} and {\tt timestamp}.  When these arguments are provided, they will be used instead of the current user and
    current time as the metadata for history revisions.}
