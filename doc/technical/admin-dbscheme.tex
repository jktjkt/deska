...
\begin{itemize}
    ...
    \item have unique, not null column name of identifier type; for embed tables should be unique pair uid and referencing column
    \item assign created sequence's nextval as the default value of the column uid
\end{itemize}
...
Foreign keys have to have set ON DELETE NO ACTION, it's the default option. Foreign keys also needs to be set to DEFERRABLE.
...
\subsection{Embed into}
Embed into relation needs adding the foreign key constraint. This constraint have to:
\begin{itemize}
    \item refer to module that will be installed before this one
    \item have referring column of bigint type
