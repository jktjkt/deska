...
BEGIN
	...
	PERFORM startchangeset();
	PERFORM host_set_note_host('hpv1', 'host hpv1');
	PERFORM host_add('hpv2');
	PERFORM host_set_note_host('hpv2', 'host hpv2');

	PREPARE retnames AS SELECT note_host FROM host_get_data('hpv1');
	...
	PREPARE retnotes AS SELECT note_host FROM host_get_data('hpv1');
